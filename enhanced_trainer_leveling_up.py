...
class EnhancedStockfishTrainer:
    def __init__(self,
        ...
        self.current_sf_skill = 1
        self.winrate_threshold = 0.60   # % needed to rank up
        self.max_sf_skill = 20          # Stockfish max skill
    ...
    # IO Helpers
    # -------------------------
    def _set_stockfish_skill(self, skill):
        """Try to set Stockfish skill (Skill Level and UCI_Elo). If configure fails, restart engine."""
        try:
            skill = int(skill)
        except Exception:
            print(f"⚠ Invalid skill value: {skill}")
            return

        self.current_sf_skill = skill
        elo = max(800, 800 + skill * 100)

        cfg = {}
        # Try both common options (some builds expose one or the other)
        cfg["UCI_LimitStrength"] = True
        cfg["UCI_Elo"] = int(elo)
        cfg["Skill Level"] = skill

        if not self.stockfish_engine:
            # Not running yet — engine will be started elsewhere with the supplied skill
            return

        # Try to configure. If configure fails, restart engine with the requested skill.
        try:
            # filter keys that engine actually accepts to reduce noisy exceptions
            accepted = {}
            try:
                opts = getattr(self.stockfish_engine, "options", {}) or {}
            except Exception:
                opts = {}
            for k, v in cfg.items():
                if k in opts:
                    accepted[k] = v
            # always attempt accepted keys first
            if accepted:
                try:
                    self.stockfish_engine.configure(accepted)
                except Exception:
                    # fall through to engine restart
                    raise
            else:
                # engine doesn't expose these options: attempt configure anyway (some builds accept them)
                try:
                    self.stockfish_engine.configure(cfg)
                except Exception:
                    raise

            # success: print status
            print(f"🤖 Stockfish configured: Skill Level={skill}, UCI_Elo~={elo}")
            return
        except Exception:
            # restart engine as a fallback
            try:
                try:
                    self.stockfish_engine.quit()
                except Exception:
                    pass
                self.stockfish_engine = init_stockfish(self.stockfish_path, skill_level=skill, threads=self.stockfish_threads)
                if self.stockfish_engine:
                    print(f"🤖 Stockfish restarted with Skill Level={skill} (approx ELO={elo})")
                else:
                    print("⚠ Failed to restart Stockfish with new skill setting.")
            except Exception as e:
                print(f"⚠ Error while restarting Stockfish: {e}")
    ...
    def run_for_minutes(self,
        ...
        cycle = 0
                ...
                # start engine with requested skill
                # start engine with requested skill
        ...
        # record current skill and try to ensure engine is configured accordingly
        self.current_sf_skill = int(stockfish_skill)
        try:
            # try to set it explicitly (safe wrapper)
            self._set_stockfish_skill(self.current_sf_skill)
        except Exception:
            pass

        # Print active Stockfish skill/ELO in a robust manner
        try:
            opts = getattr(self.stockfish_engine, "options", {}) or {}
            skill_str = None
            elo_str = None
            if "Skill Level" in opts:
                try:
                    skill_str = str(opts["Skill Level"].default_value)
                except Exception:
                    skill_str = str(self.current_sf_skill)
            if "UCI_Elo" in opts:
                try:
                    elo_str = str(opts["UCI_Elo"].default_value)
                except Exception:
                    elo_str = str(max(800, 800 + self.current_sf_skill * 100))

            if skill_str is not None and elo_str is not None:
                print(f"🤖 Stockfish running — Skill Level: {skill_str} | approx ELO setting: {elo_str}")
            elif skill_str is not None:
                print(f"🤖 Stockfish running — Skill Level: {skill_str}")
            elif elo_str is not None:
                print(f"🤖 Stockfish running — approx ELO setting: {elo_str}")
            else:
                # fallback print using our tracked value
                print(f"🤖 Stockfish running — Skill Level (tracked): {self.current_sf_skill} | approx ELO: {max(800, 800 + self.current_sf_skill * 100)}")
        except Exception as e:
            print(f"⚠ Could not read Stockfish options: {e}")
        ...
        try:
            while time.time() < end_time:
                ...
                # Level up Stockfish if bot is too strong
                                # Level up Stockfish if bot is too strong
                if winrate_sf > self.winrate_threshold and self.current_sf_skill < self.max_sf_skill:
                    new_skill = self.current_sf_skill + 1
                    print(f"🔼 Bot winrate {winrate_sf*100:.1f}% > {self.winrate_threshold*100:.1f}% — increasing Stockfish to skill {new_skill}")
                    self._set_stockfish_skill(new_skill)
